...
import { MASTER_CHEF, MASTER_CHEF_V2, NATIVE_TOKEN_ADDRESS, PHO_TOKEN } from '../../constants/addresses'
...
import Deposit, { phoardData } from './deposit'
import { BLOCK_TIME, TOKEN_PHO_PER_BLOCK } from './FarmList'
...
export const LP_WETH_PHO = '0x38739ff518614fd2cd36f5b3a6e561faf7791dd1'
...
const FarmListItem = ({ farm, farmLength }) => {
  ...
  const phoTokenContract = useTokenContractWeb3(PHO_TOKEN[`${chainId}`])
  ...
  // ]?.toExact()
  // cach 2
  // console.log('LP_WETH_PHO: ', LP_WETH_PHO)
  // const balanceNative = useWETHBalances([LP_WETH_PHO])[LP_WETH_PHO]?.toExact()
  // cach 3
  ...
  const fetchData = async () => {
    ...
    const phoRewardHex = account
      ? farm.active && farm.phoEarning
        ? await (farmingContractV2.pendingPho(farm.pid, account) || 0)
        : !checkClaim
        ? await (farmingContract.pendingPho(farm.pid, account) || 0)
        : 0
      : 0

    const totalStakedForFarmToken = farm.active ? farmingContractV2 && (await farmingContractV2.totalPhoStaked()) : 0
    ...
    const phoReward = new BigNumber(convertToNumber(phoRewardHex, 18)).times(phoPrice).toNumber() || 0
    ...
    const token1RewardUsd =
      new BigNumber(convertToNumber(token1RewardHex, farm.pair.quoteToken.decimals))
        ...
        .toNumber() || 0
    const apy = apyTken0 + apyTken1 + apyTokenPho
    ...
    if (farm.type === FarmTypeEnum.TOKEN) {
      const rewardAllBlock = farm.active
        ? farmingContractV2 && (await farmingContractV2.phoPerBlock())
        : farmingContract && (await farmingContract.phoPerBlock())
    ...
    setData({
      ...
      totalAmountReward,
      remainingReward,
      phoTokenReward: new BigNumber(convertToNumber(phoRewardHex, 18)).toNumber(),
      tokenPriceUSD,
      tokenFarmAPY,
      checkClaim,
    })
  ...
  const getLpTokenPerYear = () => {
    if (networkSupport.supportedChainIds.includes(chainId)) {
      ...
      return blockPerYear.times(TOKEN_PHO_PER_BLOCK[chainId])
    } else return 0
  }

  const getFarmAPYPho = async () => {
    try {
      let phoPerYear = farm.phoEarning ? new BigNumber(getLpTokenPerYear()) : new BigNumber(0)
      let apyTokenPho = farm.phoEarning ? await getApyForToken(PHO_TOKEN[`${chainId}`], phoPerYear) : 0
  ...
  const getApyForToken = useCallback(
    async (token: string, tokenPerYear: BigNumber) => {
      ...
      if (
        token == PHO_TOKEN[chainId] &&
        farm.pair.token.id != PHO_TOKEN[chainId] &&
        farm.pair.quoteToken.id != PHO_TOKEN[chainId]
      ) {
        const [phoInLp, quoteInLp] = await Promise.all([
          phoTokenContract.options.address &&
            phoTokenContract.methods.balanceOf(farm?.lpAddressPhoVsQuote[`${chainId}`]).call(),
          quoteTokenContract.options.address && farm.pair.quoteToken.id === NATIVE_TOKEN_ADDRESS
            ? useWETHBalances([farm?.lpAddressPhoVsQuote[`${chainId}`]])[
                farm?.lpAddressPhoVsQuote[`${chainId}`]
              ]?.toExact()
            : quoteTokenContract.methods.balanceOf(farm?.lpAddressPhoVsQuote[`${chainId}`]).call(),
        ])
        tokenBalanceLP = toFixed(
          Number(
            new BigNumber(convertToNumber(quoteTokenBalanceLP, farm.pair.quoteToken.decimals))
              .times(convertToNumber(phoInLp, 18))
      ...
      const yearlyPHORewardAllocation = new BigNumber(farmInfo.allocPoint.toString())
        .times(tokenPerYear.toNumber())
        .div(new BigNumber(totalAllocPoint.toString()))
      const apy = tokenPriceVsQuote.times(yearlyPHORewardAllocation).div(lpTotalInQuoteToken).times(100)
  ...
  const handleClaim = async () => {
    const estimate = farm.active ? farmingContractV2.estimateGas.stake : farmingContractV2.estimateGas.claimOldPho
    const method = farm.active ? farmingContractV2.stake : farmingContractV2.claimOldPho
    ...
    await estimate(...args)
      .then((estimatedGasLimit) =>
        method(...args, {
          ...
          addTransaction(response, {
            summary: `Claim ${farm.phoEarning ? 'PHO' : ''} ${
              farm.phoEarning
                ? farm.pair.token.earning
  ...
  const dataApyTokenPho = data?.apyTokenPho || 0
  ...
  let totalDataAPY = `<p></p><p></p>${
    ...
    dataApyTokenPho !== 0 ? `<div>${handleFarmingAPYDecimal(dataApyTokenPho)}% - PHO Farming APY</div>` : '0%'
  }`
            ...
            <div className="text-left select-none text-primary text-sm md:text-lg item-list">
                    ...
                    <NewTooltip
                      ...
                      dataValue={
                        farm.type === FarmTypeEnum.TOKEN
                          ...
                          : `${handleAdvancedDecimal(dataApyTken0 + dataApyTken1 + dataApyTokenPho, 6)}%`
                      }
                    />
                  </NumberTotal>
                  <div></div>
              ...
              <div className="tit-token">
                ...
                <div className="flex flex-col justify-center">
                  <div className="font-bold title-token py-3.5">
                    ...
                    ${i18n._(t`to earn`)} ${farm.phoEarning ? 'PHO' : ''}
                    ${
                      farm.phoEarning
                        ? farm.pair.token.earning
                  ...
                  <div className="w-full flex justify-center">
                    {farm.active ? (
                      <StatusCard>
                        {`${i18n._(t`Active`)}
                          ${
                            (farm.phoEarning && !farm.pair.token.earning && !farm.pair.quoteToken.earning) ||
                            (!farm.singleFarm && farm.endDate < timeNow)
                              ? i18n._(t`For PHO Only`)
                              : ''
                          }
                          ${
                            !farm.phoEarning && farm.pair.token.earning && !farm.pair.quoteToken.earning
              ...
              <BoxContent>
                ...
                <div className="flex justify-between items-center py-1">
                  ...
                  <CountStake>
                    <NewTooltip
                      dataValue={handleAdvancedDecimal(phoardData(data.userStake), 10)}
                      dataTip={` ${handleToFixedDataTip(phoardData(data.userStake), 10)}`}
                    />
                  </CountStake>
                </div>
                {farm.phoEarning && (
                  <div className="flex justify-between py-1 items-center">
                    <TextStake>{i18n._(t`PHO reward `)}</TextStake>
                    <CountStake>
                      {/* ${phoardData(data?.token0Reward?.toFixed(2))} */}
                      <NewTooltip
                        dataValue={handleAdvancedDecimal(phoardData(data?.phoTokenReward), 10)}
                        dataTip={` ${handleToFixedDataTip(phoardData(data?.phoTokenReward), 10)}`}
                      />
                    </CountStake>
                  </div>
                )}
                {farm.pair.token.earning && (
                  <>
                    <div className="flex justify-between py-1 items-center">
                      ...
                      <CountStake>
                        <NewTooltip
                          dataTip={'$' + handleToFixedDataTip(phoardData(data?.token0RewardUsd), 10)}
                          dataValue={` $${handleAdvancedDecimal(phoardData(data?.token0RewardUsd), 10)}`}
                        />
                      </CountStake>
                    </div>
                    <div className="flex justify-between py-1 items-center">
                      ...
                      <CountStake>
                        <NewTooltip
                          dataTip={handleToFixedDataTip(phoardData(data?.token0Reward), 10)}
                          dataValue={` ${handleAdvancedDecimal(phoardData(data?.token0Reward), 10)}`}
                        />
                      </CountStake>
                    </div>
                  </>
                  ...
                  <>
                    <div className="flex justify-between py-1 items-center">
                      ...
                      <CountStake>
                        {/* ${phoardData(data?.token0Reward?.toFixed(2))} */}
                        <NewTooltip
                          dataValue={'$' + handleAdvancedDecimal(phoardData(data?.token1RewardUsd), 10)}
                          dataTip={` $${handleToFixedDataTip(phoardData(data?.token1RewardUsd), 10)}`}
                        />
                      </CountStake>
                    </div>
                    <div className="flex justify-between py-1 items-center">
                      ...
                      <CountStake>
                        <NewTooltip
                          dataTip={handleToFixedDataTip(phoardData(data?.token1Reward), 10)}
                          dataValue={` ${handleAdvancedDecimal(phoardData(data?.token1Reward), 10)}`}
                        />
                      </CountStake>
                    </div>
                  </>
                  ...
                  <div className="flex justify-between py-1 items-center">
                    ...
                    <CountStake>
                      <NewTooltip
                        ...
                        dataValue={` $${handleAdvancedDecimal(phoardData(data?.totalAmountReward), 10)}`}
                      />
                    </CountStake>
                  </div>
                )}
                {(farm.pair.token.limitedAmount !== undefined || farm.pair.quoteToken.limitedAmount !== undefined) && (
                  <div className="flex justify-between py-1 items-center">
                    ...
                    <CountStake>
                      <NewTooltip
                        dataTip={'$' + handleToFixedDataTip(phoardData(data?.remainingReward), 10)}
                        dataValue={` $${handleAdvancedDecimal(phoardData(data?.remainingReward), 10)}`}
                      />
                    </CountStake>
                  </div>
                )}
                      ...
                      >
                        ...
                        <div className="desc">
                          {farm.type === FarmTypeEnum.TOKEN
                            ...
                            : farm.type === FarmTypeEnum.NFT
                            ? 'NFT'
                            : `phoLP-${farm.pair.token.symbol}-${farm.pair.quoteToken.symbol}`}
                        </div>
                      </button>
                      {isOpenDeposit && (
                        <Deposit
                          ...
                          apy={farm.type === FarmTypeEnum.TOKEN ? data.tokenFarmAPY : data?.apyTokenPho}
                      ...
                      >
                        ...
                        <div className="desc">
                          {`${farm.phoEarning ? 'PHO' : ''}
                              ${
                                farm.pair.token.earning
                                  ? data.token0Reward === 0 && data.phoTokenReward !== 0
                                    ? ''
                                    : i18n._(t`and`) + ' ' + farm.pair.token.symbol
                                  : ''
                              }
                              ${
                                farm.pair.quoteToken.earning
                                  ? data.token1Reward === 0 && data.phoTokenReward !== 0
                                    ? ''
                                    : i18n._(t`and`) + ' ' + farm.pair.quoteToken.symbol
                                  : ''
                              } ${i18n._(t`reward`)}`}
                    ...
                    <div className="flex flex-row items-center btn-container mb-36">
                      ...
                      >
                        <div className="label capitalize">
                          ...
                          {`${
                            farm.type === FarmTypeEnum.TOKEN ? ' PHO' : farm.type === FarmTypeEnum.NFT ? ' NFT' : ''
                          }`}
                        </div>
                        <div className="desc">
