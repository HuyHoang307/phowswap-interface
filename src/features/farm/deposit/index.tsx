...
import { MASTER_CHEF, MASTER_CHEF_V2, PHO_TOKEN } from '../../../constants/addresses'
...
export const phoardData = (data: any) => {
...
export default function Deposit({
  ...
  const [tokenPhoPrice, setTokenPhoPrice] = useState(0)
  ...
  useEffect(() => {
    setCountValue(null)
    const getTokenPhoPrice = async () => {
      const [phoPrice, tokenBalance] = await Promise.all([0, lpTokenContract.methods.balanceOf(account).call()])
      ...
      setTokenPhoPrice(phoPrice)
    }

    isOpenDeposit && getTokenPhoPrice()
  ...
  const calculateSdcEarnedPerThousandDollars = ({ numberOfDays, farmApy, tokenPrice }) => {
    ...
    //   Calculate the starting PHO balance with a dollar balance of $1000.
    ...
    // To get the PHO earned, deduct the amount after compounding (finalAmount) from the starting PHO balance (principal)
  ...
  const tokenPhoEarnedPerThousand1D = calculateSdcEarnedPerThousandDollars({
    numberOfDays: 1,
    farmApy,
    tokenPrice: tokenPhoPrice,
  })
  const tokenPhoEarnedPerThousand30D = calculateSdcEarnedPerThousandDollars({
    numberOfDays: 30,
    farmApy,
    tokenPrice: tokenPhoPrice,
  })
  const tokenPhoEarnedPerThousand365D = calculateSdcEarnedPerThousandDollars({
    numberOfDays: 365,
    farmApy,
    tokenPrice: tokenPhoPrice,
  })

  const handlePriceAmount = (tokenPhoEarned, tokenPhoPrice) => {
    return phoardData(new BigNumber(tokenPhoEarned).times(tokenPhoPrice).toNumber()) === 0 ||
      phoardData(new BigNumber(tokenPhoEarned).times(tokenPhoPrice).toNumber()) === '0'
      ? '0.00'
      : phoardData(new BigNumber(tokenPhoEarned).times(tokenPhoPrice).toNumber())
  }
    ...
    <Modal isOpen={isOpenDeposit} onDismiss={onDismissDeposit} maxWidth={568}>
      ...
      <ContentModal>
        <div className="head-title">
          ...
          <div className="available" onClick={() => setCountValue(phoardData(userLpAmount))}>
            {`${
              farmType === FarmTypeEnum.TOKEN ? 'PHO' : `LP-${farm.pair.token.symbol}-${farm.pair.quoteToken.symbol}`
            } ${i18n._(t`Balance:`)} `}
            <StyledNewTooltip>
              <NewTooltip
                dataTip={phoardData(userLpAmount)}
                dataValue={handleAdvancedDecimal(phoardData(userLpAmount), 6)}
              />
            </StyledNewTooltip>
          </div>
        </div>
