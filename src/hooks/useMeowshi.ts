...
import { useMeowshiContract, usePhoBarContract, usePhoContract } from './useContract'
...
const useMeowshi = (sushi: boolean) => {
  ...
  const sushiContract = usePhoContract(true)
  const barContract = usePhoBarContract(true)
  ...
  const meowPho = useCallback(
    async (amount: BalanceProps | undefined) => {
      if (amount?.value) {
        try {
          const tx = await meowshiContract?.meowPho(account, amount?.value)
          addTransaction(tx, { summary: 'Enter Meowshi' })
          return tx
        } catch (e) {
          return e
  ...
  const unmeowPho = useCallback(
    async (amount: BalanceProps | undefined) => {
      if (amount?.value) {
        try {
          const tx = await meowshiContract?.unmeowPho(account, amount?.value)
          addTransaction(tx, { summary: 'Leave Meowshi' })
          return tx
        } catch (e) {
          return e
