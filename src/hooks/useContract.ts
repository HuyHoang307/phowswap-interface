...
import PHO_ABI from '../constants/abis/sushi.json'
...
import PHO_FARMING_ABI from '../constants/abis/pho-farming.json'
...
const PHO_FARMING_ADDRESS = '0x8d1f6DA6eB9AA76D4886640730558E3222e68CfE'
...
export function usePhoContract(withSignerIfPossible = true): Contract | null {
  const { chainId } = useChainId()
  return useContract(chainId && PHO_ADDRESS[chainId], PHO_ABI, withSignerIfPossible)
}
...
export function usePhoBarContract(withSignerIfPossible?: boolean): Contract | null {
  const { chainId } = useChainId()
...
export function usePhoFarmingContract(): Contract | null {
  return useContract(PHO_FARMING_ADDRESS, PHO_FARMING_ABI, false)
}
...
export function useFarmingContract(withSignerIfPossible?: boolean): Contract | null {
  const { chainId } = useChainId()
  let address = PHO_FARMING_ADDRESS
  let abi = PHO_FARMING_ABI
