import { ChainId, Currency, WNATIVE } from 'phoswap-sdk'
...
export function getCurrencyLogoUrls(currency) {
  const urls = []
  urls.push(
    `https://raw.githubusercontent.com/Pho-Swap/pho-swap-interface/master/public/images/icons/${currency.symbol.toLowerCase()}.png`
  )
  return urls
}
...
const CurrencyLogo: FunctionComponent<CurrencyLogoProps> = ({
  ...
  return (
    <Logo
      width={size}
      height={size}
      alt={currency?.chainId != 4 && currency?.symbol == 'SUSHI' ? 'PHO' : currency?.symbol}
      className={className}
      {...rest}
    />
  )
